...
async function seedDemoConsumptions(userId) {
    ...
    const findFood = (nameEN) => foods.find((f) => f.nameEN.toLowerCase().includes(nameEN.toLowerCase()));
    const now = new Date();
    ...
    for (const meal of meals) {
        ...
        for (const item of meal.items) {
            ...
            if (!food || !food.calories || !food.protein || !food.carbs || !food.fat)
                continue;
