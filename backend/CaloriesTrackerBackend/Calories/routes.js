...
export function getPublicCaloriesRoutes() {
    const router = Router();
    ...
    router.get('/search-food', async (req, res) => {
        try {
            ...
            const results = await CaloriesFoodController.searchFoodbyQuery(validatedQuery.query, validatedQuery.lang, validatedQuery.categoriaId);
...
export function getPrivateCaloriesRoutes() {
    const router = Router();
    router.get('/consumptions', async (req, res) => {
        try {
            const validatedQuery = GetConsumptionsQuerySchema.parse(req.query);
            const result = await CaloriesFoodController.getConsumptions(req.user.id, // Assuming user is attached to req by JWT middleware
            validatedQuery.page, validatedQuery.limit, validatedQuery.startDate, validatedQuery.endDate);
            res.status(200).json(result);
        }
        catch (error) {
            if (error instanceof ZodError) {
